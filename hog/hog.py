...
def roll_dice(num_rolls, dice=six_sided):
    ...
    if num_rolls == 1:
        return dice()
    out = dice()
    nxt = roll_dice(num_rolls-1, dice)
    if nxt == 1 or out == 1:
        return 1  # pig out
    return out+nxt
...
def is_swap(player_score, opponent_score):
    ...
    return abs(p_tens-p_ones) == abs(o_tens-o_ones)
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    "*** YOUR CODE HERE ***"
    def say(get, p):
        print(get, " point(s)! That's the biggest gain yet for Player ", p)

    def high(score0, score1):
        if who == 0:
            get = score0 - previous_score
            if get > previous_high:
                say(get, 0)
                return announce_highest(who, get, score0)
            else:
                return announce_highest(who, previous_high, score0)
        else:
            get = score1 - previous_score
            if get > previous_high:
                say(get, 1)
                return announce_highest(who, get, score1)
            else:
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    times = 0
    for i in range(1, 11):
        t = roll_dice(i, make_averaged(dice, num_samples))
        if t > m:
            m = t
            times = i
    return times
