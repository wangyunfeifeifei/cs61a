...
def group_by_centroid(restaurants, centroids):
    ...
    for item in restaurants:
        ce = find_closest(restaurant_location(item), centroids)
        ls.append([centroids.index(ce), item])
...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    "*** YOUR CODE HERE ***"
    return [mean([restaurant_location(item)[0] for item in cluster]),
            mean([restaurant_location(item)[1] for item in cluster])]
    # END Question 5
...
def find_predictor(user, restaurants, feature_fn):
    ...
    sxx = sum([(x-m_x)**2 for x in xs])
    syy = sum([(y-m_y)**2 for y in ys])
    sxy = sum([(xs[i]-m_x)*(ys[i]-m_y) for i in range(len(xs))])
    b = sxy / sxx
    a, r_squared = m_y - b * m_x, sxy**2 / (sxx * syy)
    # END Question 7
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        ...
        if restaurant in reviewed:
            ret[name] = user_rating(user, name)
        else:
            ret[name] = predictor(restaurant)
    return ret
    # END Question 9
