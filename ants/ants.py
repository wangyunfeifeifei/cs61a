...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 7
        return True if self.place.ant and self.place.ant.blocks_path else False
        # END Problem 7
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        p = self.place
        while p is not None and len(p.bees) == 0:
            p = p.entrance
        return random_or_none(p.bees) if p is not None and p is not hive else None
        # END Problem 3 and 4
...
class LongThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        d, p = 0, self.place
        while p and d < self.min_range:
            d, p = d+1, p.entrance
        if p is None or p is hive:
            return None
        while p and len(p.bees) == 0:
            p = p.entrance
        return random_or_none(p.bees) if p is not None and p is not hive else None
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        self.armor -= amount
        if self.armor <= 0:
            for ins in self.place.bees[:]:
                ins.reduce_armor(self.damage)
            self.place.remove_insect(self)
        # END Problem 5
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting > 0:
            self.digesting -= 1
        elif self.place.bees and not self.digesting:
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        self.is_true_queen = QueenAnt.queen
        if QueenAnt.queen:
    ...
    def action(self, colony):
        ...
        if self.is_true_queen:
            ...
            while p.exit:
                ...
                if not ant:
                    continue
                if not QueenAnt.has_buff(ant):
                    QueenAnt.buff(ant)
                if ant.is_container and ant.contained_ant \
                    and not QueenAnt.has_buff(ant.contained_ant):
                    QueenAnt.buff(ant.contained_ant)
        else:
    ...
    @staticmethod
    def has_buff(ant: Ant) -> bool:
        return hasattr(ant, 'queen_buff')
